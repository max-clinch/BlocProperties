...
async function validateArtifacts(artifactsPath, sourcesPath) {
    ...
    return (0, upgrades_core_1.validate)(output, srcDecoder, solcVersion);
}
...
function reconstructSolcInputOutput(artifacts) {
    ...
    for (const artifact of artifacts) {
        ...
        if (!versionMatch) {
            throw new upgrades_core_1.UpgradesError('Unknown or nightly compiler version');
        }
        ...
        if (solcVersion === undefined) {
            ...
            throw new upgrades_core_1.UpgradesError('Multiple Solidity versions found');
        }
    ...
    for (const source in output.sources) {
        ...
        for (const importDir of (0, utils_1.findAll)('ImportDirective', ast)) {
            ...
            if (importedUnitId === undefined) {
                ...
                throw new upgrades_core_1.UpgradesError(`Artifacts are from different compiler runs\n` +
...
function reconstructLinkReferences(bytecode) {
    ...
    for (let index = 0; index < bytecode.length;) {
        ...
        (_a = linkReferences['*'])[libName] ?? (_a[libName] = []);
...
async function getLinkedBytecode(Contract, provider) {
    ...
    const networkInfo = Contract.networks?.[networkId];
